...
import { ScrollView, Text, View, ActivityIndicator, Alert, KeyboardAvoidingView, Platform, Keyboard, TouchableOpacity } from 'react-native';
...
export default function FoodDetailsScreen() {
  ...
  const { data: barcodeFood, isLoading: isLoadingBarcode, error: barcodeError } = useFoodBarcode(params.barcode || null);
  ...
  useEffect(() => {
    if (barcodeFood) {
      ...
      try {
        ...
        if (params.entryId && parsed.id && (!parsed.units || parsed.units.length === 0)) {
              ...
              >
                 <View className="mb-6">
                     ...
                     {food.brand && food.brand.length > 0 && (
                         <Text className="text-base text-muted-foreground mt-1">{food.brand}</Text>
                     )}
                 </View>
                    ...
                    >
                        {isGlobalBusy ? (
                            ...
                            <Text className="text-white text-lg font-bold">
                                {params.entryId ? t('manualEntry.save') : t('foodDetails.addToDiary')}
                            </Text>
                        )}
                    </TouchableOpacity>
                </View>
